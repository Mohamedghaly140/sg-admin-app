...
- [ ] `hooks/use-orders-params.ts` — `status`, `paymentMethod`, `isPaid`, `search`, `from`, `to`, `page`, `limit`.
- [ ] `queries/get-orders.ts`; table per the screen spec (humanOrderId, customer w/ "Guest" fallback, status + paid badges, total, date).
- [ ] Toolbar: debounced search, status/isPaid selects, date-range picker.

### 2. Detail (`/orders/[id]`)

- [ ] `queries/get-order.ts`; unknown id → `notFound()`.
- [ ] Customer panel: registered (`user` + `shippingAddress`) **xor** guest (`anon*` snapshot + "Guest order" badge).
- [ ] Items table with price snapshots + product links; totals card (display-only, all server-computed, coupon line when present).

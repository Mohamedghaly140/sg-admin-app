...
- **First attempt (reverted)**: defaulting `nativeButton` to `false` whenever a `render` prop was present silenced the warning but made Base UI stamp `role="button" tabindex="0"` onto the `<a>` — stripping link semantics from navigation controls (an a11y regression flagged in follow-up review).
- **Fix applied**: navigation buttons no longer route through the shared `Button`. The three call sites now style a real Next.js `<Link>` directly with `buttonVariants()` — `features/products/index.tsx:55`, `app/not-found.tsx:14`, `app/(admin)/not-found.tsx:13`. This yields a true `<a>` (proper link semantics for assistive tech) with button styling, and no Base UI Button wraps a non-button anymore. The `nativeButton` auto-default was removed from `components/ui/button.tsx`. The row-action/breadcrumb/nav `render`s are on *other* Base UI primitives and were never affected.

...
|---|---|---|
| Create user | `createUser` → `POST /admin/users` | Dialog: separate `firstName` + `lastName` inputs (each trimmed, non-empty; composed full name ≤ 120 code points to match the backend's surrogate-aware check), `email`, `phone` (Egyptian `+2010…`), `password` (≥ 8 — **never echoed or stored**; the operator hands it over out-of-band), `role` select. Send both fields as-is — **not** the former single `name` (the backend passes them to Clerk untouched, avoiding the `last_name` rejection on single-token names). `422 VALIDATION_ERROR` carries Clerk's reason (duplicate email/phone, weak/breached password) → show `message` and any `errors[]` on fields. |
